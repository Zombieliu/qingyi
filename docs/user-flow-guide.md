...
> 说明：本流程为“前端触发的真人操作模拟”，使用两个 Passkey 地址分别扮演「用户」与「陪玩」，同一地址禁止接自己发的单。
> 录屏与截图来自最近一次链上 E2E 流程执行，时间：2026-02-08。

## 0. 前置条件
- 浏览器环境支持 Passkey（HTTPS 或 localhost）。
- 已配置链上环境（`NEXT_PUBLIC_CHAIN_ORDERS=1`）。
- 具备链上测试资金（Sui Testnet）。
- 准备两个 Passkey 地址（用户 / 陪玩各一个）。

---

## 1. 用户 Passkey 注册/登录
首次进入系统，点击 Passkey 登录按钮创建钱包并登录（用户账号）。
...
## 6. 链上订单状态检查（用户）
切换到「展示」页（公共接单池），点击“刷新链上订单”查看订单状态（页面每 30 秒自动刷新一次，并显示上次刷新时间）。
...
## 7. 打手接单（陪玩登录并付押金）
切换到陪玩账号（不同 Passkey 地址）后，点击“付押金接单”（会弹出二次确认）。
...
- 成功后订单状态变为“押金已锁定”。
- 表示陪玩已接单。
- 接单后可看到用户的游戏名与ID；若未立即显示，点击“加载用户信息”即可同步并复制。
- 同一地址无法接自己发的单。

---

## 8. 用户确认完成
切回用户账号，点击“确认完成”（会弹出二次确认），订单进入待结算状态。
...
- 陪玩接单必须使用不同 Passkey 地址（系统已限制同地址接单）。
