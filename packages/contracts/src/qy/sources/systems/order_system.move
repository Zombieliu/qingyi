...
module qy::order_system {
  ...
  public fun create_order(
    ...
    assert!(!order::has(dapp_hub, dapp_key::to_string(), order_id), E_ORDER_EXISTS);
    ...
    order::set_struct(dapp_hub, dapp_key::to_string(), order_id, order_struct, ctx);
  ...
  public fun lock_deposit(dapp_hub: &mut DappHub, order_id: u64, ctx: &mut TxContext) {
    ...
    ledger_system::debit_balance(dapp_hub, companion, deposit, ctx);
  ...
  public fun finalize_no_dispute(dapp_hub: &mut DappHub, order_id: u64, clock: &Clock, ctx: &mut TxContext) {
    ...
    ledger_system::add_balance(dapp_hub, order::companion(&ord), companion_service + order::vault_deposit(&ord), ctx);
  ...
  public fun resolve_dispute(
    ...
    ledger_system::add_balance(dapp_hub, order::user(&ord), refund_service + user_from_deposit, ctx);
    ledger_system::add_balance(dapp_hub, order::companion(&ord), companion_service + companion_deposit, ctx);
  ...
  public fun cancel_order(dapp_hub: &mut DappHub, order_id: u64, ctx: &mut TxContext) {
    ...
    if (order::status(&ord) == STATUS_PAID) {
      ledger_system::add_balance(dapp_hub, order::user(&ord), order::vault_service(&ord), ctx);
      order::update_vault_service(&mut ord, 0);
      order::update_status(&mut ord, STATUS_CANCELLED);
      order::set_struct(dapp_hub, dapp_key::to_string(), order_id, ord, ctx);
      return
