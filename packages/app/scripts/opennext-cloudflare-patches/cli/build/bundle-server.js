...
export async function bundleServer(buildOpts, projectOpts) {
    ...
    // The Next server lazily requires middleware-manifest.json via dynamic require,
    // which is not resolvable in workerd module graphs. Use loadManifest instead.
    const nextServerPath = path.join(outputDir, "node_modules", "next", "dist", "server", "next-server.js");
    ...
    const result = await build({
        ...
        alias: {
            ...
            // Not available in workerd and only used for colorized terminal formatting
            tty: path.join(buildOpts.outputDir, "cloudflare-templates/shims/tty.js"),
            "node:tty": path.join(buildOpts.outputDir, "cloudflare-templates/shims/tty.js"),
            // Workerd does not expose node:vm
            vm: path.join(buildOpts.outputDir, "cloudflare-templates/shims/vm.js"),
            "node:vm": path.join(buildOpts.outputDir, "cloudflare-templates/shims/vm.js"),
            // Child processes are unavailable in workers (Prisma Node engine path)
            "child_process": path.join(buildOpts.outputDir, "cloudflare-templates/shims/child-process.js"),
            "node:child_process": path.join(buildOpts.outputDir, "cloudflare-templates/shims/child-process.js"),
            ...
            // Workerd does not allow mutating node:timers module exports
            "next/dist/server/node-environment-extensions/fast-set-immediate.external": path.join(buildOpts.outputDir, "cloudflare-templates/shims/empty.js"),
            "next/dist/server/node-environment-extensions/fast-set-immediate.external.js": path.join(buildOpts.outputDir, "cloudflare-templates/shims/empty.js"),
