...
async function generateBundle(name, options, fnOptions, codeCustomization) {
    ...
    // Next 16 standalone output can omit node_modules. Ensure full next/dist
    // exists in .open-next before bundling the server adapter.
    const findPackageInParents = (packageName) => {
        const candidates = [];
        let currentDir = appPath;
        while (true) {
            candidates.push(path.join(currentDir, "node_modules", packageName));
        ...
        const monorepoCandidate = path.join(monorepoRoot, "node_modules", packageName);
        if (!candidates.includes(monorepoCandidate)) {
            candidates.push(monorepoCandidate);
        }
        return candidates;
    };
    const nextDistDest = path.join(options.outputDir, "node_modules", "next", "dist");
    const nextDistCandidates = findPackageInParents("next").map((pkgPath) => path.join(pkgPath, "dist"));
    const hasNextDist = (distPath) => {
        return (fs.existsSync(path.join(distPath, "server", "node-environment.js")) &&
            fs.existsSync(path.join(distPath, "shared", "lib", "utils.js")));
    };
    const nextDistSrc = nextDistCandidates.find(hasNextDist);
    if (nextDistSrc && !hasNextDist(nextDistDest)) {
        fs.mkdirSync(path.dirname(nextDistDest), { recursive: true });
        fs.cpSync(nextDistSrc, nextDistDest, { recursive: true });
    }
    const styledJsxDest = path.join(options.outputDir, "node_modules", "styled-jsx");
    const hasStyledJsxDist = (pkgPath) => fs.existsSync(path.join(pkgPath, "dist", "index.js")) ||
        fs.existsSync(path.join(pkgPath, "dist", "index", "index.js"));
    const styledJsxSrc = findPackageInParents("styled-jsx").find(hasStyledJsxDist);
    if (styledJsxSrc && !hasStyledJsxDist(styledJsxDest)) {
        fs.mkdirSync(path.dirname(styledJsxDest), { recursive: true });
        fs.cpSync(styledJsxSrc, styledJsxDest, { recursive: true });
    }
    // Next 16 fast-set-immediate mutates node:timers exports, which is read-only on workerd.
    // Remove the extension import to avoid runtime 1101 crashes.
    const nodeEnvironmentPath = path.join(options.outputDir, "node_modules", "next", "dist", "server", "node-environment.js");
    if (fs.existsSync(nodeEnvironmentPath)) {
        const source = fs.readFileSync(nodeEnvironmentPath, "utf8");
        const patched = source.replace(/require\(["']\.\/node-environment-extensions\/fast-set-immediate\.external["']\);\r?\n?/, "");
        if (patched !== source) {
            fs.writeFileSync(nodeEnvironmentPath, patched);
        }
    }
    // Ensure middleware manifest is loaded via loadManifest (workerd cannot resolve dynamic require of JSON).
    const nextServerPath = path.join(options.outputDir, "node_modules", "next", "dist", "server", "next-server.js");
    if (fs.existsSync(nextServerPath)) {
        const source = fs.readFileSync(nextServerPath, "utf8");
        const patched = source.replace("const manifest = require(this.middlewareManifestPath);", "const manifest = (0, _loadmanifestexternal.loadManifest)(this.middlewareManifestPath);");
        if (patched !== source) {
            fs.writeFileSync(nextServerPath, patched);
        }
    }
    // Build Lambda code
