...
const CF_NODE_BOUND_ROUTE_POLICY = (process.env.CF_NODE_BOUND_ROUTE_POLICY || "off").trim();
const CF_NODE_BOUND_ROUTE_PREFIXES = (
  process.env.CF_NODE_BOUND_ROUTE_PREFIXES ||
  "/api/announcements,/api/health,/api/players,/api/referral/leaderboard,/api/vip/tiers"
)
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
export async function middleware(req: NextRequest) {
  ...
  // Optional Cloudflare rollout policy:
  // block selected node/db-bound routes with explicit 503 while migration is ongoing.
  if (pathname.startsWith("/api/") && CF_NODE_BOUND_ROUTE_POLICY === "block") {
    const shouldBlock = CF_NODE_BOUND_ROUTE_PREFIXES.some((prefix) =>
      matchesRoutePrefix(pathname, prefix)
    );
    if (shouldBlock) {
      return NextResponse.json(
        {
          error: "edge_route_temporarily_disabled",
          message: "Route temporarily disabled on edge runtime during DB migration",
          route: pathname,
        },
        { status: 503, headers: { "x-edge-policy": "node-bound-route-block" } }
      );
    }
  }
