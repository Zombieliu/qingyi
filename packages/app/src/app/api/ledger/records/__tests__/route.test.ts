...
vi.mock("@/lib/admin/admin-store", () => ({
  queryLedgerRecords: mocks.queryLedgerRecords,
}));
...
describe("GET /api/ledger/records", () => {
  ...
  it("returns 400 for invalid address", async () => {
    ...
    const req = new Request("http://localhost/api/ledger/records?address=bad");
  ...
  it("returns auth error when user auth fails", async () => {
    ...
    const req = new Request(`http://localhost/api/ledger/records?address=${VALID_ADDRESS}`);
    const res = await GET(req);
    expect(res.status).toBe(401);
  });

  it("returns ledger records", async () => {
    mocks.requireUserAuth.mockResolvedValue({ ok: true, address: VALID_ADDRESS });
    const result = { items: [{ id: "L-1" }], total: 1, page: 1, pageSize: 20 };
    mocks.queryLedgerRecords.mockResolvedValue(result);
    const req = new Request(`http://localhost/api/ledger/records?address=${VALID_ADDRESS}`);
    const res = await GET(req);
  ...
  it("respects page and pageSize params", async () => {
    mocks.requireUserAuth.mockResolvedValue({ ok: true, address: VALID_ADDRESS });
    mocks.queryLedgerRecords.mockResolvedValue({ items: [], total: 0 });
    const req = new Request(
      `http://localhost/api/ledger/records?address=${VALID_ADDRESS}&page=3&pageSize=10`
    );
    await GET(req);
    expect(mocks.queryLedgerRecords).toHaveBeenCalledWith({
