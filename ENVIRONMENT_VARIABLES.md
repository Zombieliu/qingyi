...
- CHAIN_ORDER_AUTO_COMPLETE_HOURS (链上订单自动完成阈值，单位小时)
- CHAIN_ORDER_AUTO_COMPLETE_MAX (单次自动完成最大订单数)
- CHAIN_ORDER_AUTO_FINALIZE_MAX (单次自动结算最大订单数)
- CHAIN_ORDER_CACHE_TTL_MS (链上订单缓存 TTL，毫秒)
- CHAIN_ORDER_MAX_CACHE_AGE_MS (链上订单缓存最大年龄，毫秒)
- CHAIN_ORDER_DEBUG ("true" 开启链上调试日志)
...
- NEXT_PUBLIC_QY_EVENT_MIN_INTERVAL_MS
...
- PAYMENT_RECONCILE_USE_STRIPE ("1" 使用 Stripe API 补对账，默认随 STRIPE_SECRET_KEY 自动启用)
- PAYMENT_RECONCILE_ALERT_WEBHOOK_URL (对账告警 webhook，推荐企业微信机器人)
- PAYMENT_RECONCILE_ALERT_ENABLED ("1" 启用告警)
- PAYMENT_RECONCILE_MISSING_THRESHOLD (缺失账本触发告警阈值，默认 1)
- PAYMENT_RECONCILE_PATCHED_THRESHOLD (修复状态触发告警阈值，默认 1)
- PAYMENT_RECONCILE_PENDING_THRESHOLD (长时间 pending 触发告警阈值，默认 5)
